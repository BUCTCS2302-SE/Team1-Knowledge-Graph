...
def translate_artifacts():
    input_file = "clean_artifacts.csv"
    output_file = "clean_artifacts_translated.csv"

    if not os.path.exists(input_file):
        print(f"❌ 找不到 {input_file}，请先运行 dataclean.py")
        return

    df = pd.read_csv(input_file)
    for col in ["title_zh", "material_zh", "type_zh", "description_zh"]:
    ...
    print(f"\n🎉 artifacts 翻译完成！结果保存到：{output_file}")


def translate_artists():
    input_file = "clean_artists.csv"
    output_file = "clean_artists_translated.csv"
    ...
    print(f"\n🔄 开始翻译 artists (共 {len(df)} 条)")

    ok = translate_column(df, "name_en", "name_zh", output_file)
    if not ok:
        return

    print(f"\n🎉 artists 翻译完成！结果保存到：{output_file}")
