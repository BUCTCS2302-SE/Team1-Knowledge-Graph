...
def fetch_and_save_china_data():
    ...
    # 筛选 ID 大于 42159 的文物
    filtered_ids = [obj_id for obj_id in object_ids if obj_id > 461499]
    print(f"筛选后 ID 大于 42159 的文物数量: {len(filtered_ids)}")
    
    if not filtered_ids:
        print("没有 ID 大于 42159 的文物")
        return
    ...
    failed_header_written = False  # 用来标记失败文件是否已经写入过表头
    
    for idx, object_id in enumerate(filtered_ids, 1):
        if idx % 5 == 0:  # 每5次请求暂停1秒，避免被封IP
            time.sleep(1)
        
        print(f"正在处理第 {idx}/{len(filtered_ids)} 个文物: ID {object_id}")
        ...
        if details:
            ...
            if len(data) >= 10:
                ...
                df.to_csv("1.csv", index=False, encoding="utf-8", mode='a', header=not header_written)
                header_written = True  
            ...
            if len(failed_ids) >= 10:
                ...
                failed_df.to_csv("failed_ids.csv", index=False, encoding="utf-8", mode='a', header=not failed_header_written)
                failed_header_written = True
                print(f"已保存 {len(failed_ids)} 条失败记录到 failed_ids.csv！")
                failed_ids = []
    ...
    if failed_ids:
        failed_df = pd.DataFrame(failed_ids)
        failed_df.to_csv("failed_ids.csv", index=False, encoding="utf-8", mode='a', header=not failed_header_written)
        print(f"成功保存 {len(failed_ids)} 条失败记录到 failed_ids.csv！")
